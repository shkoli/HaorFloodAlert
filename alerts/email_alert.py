...
Standalone reference implementation of the flood-alert email builder and
sender. The live Streamlit page (streamlit_app/pages/3_Alerts.py) has its
own inline copy of this logic for its interactive UI; this module is kept
as the documented, reusable entry point for scripted/scheduled alerts
(e.g. from daily_update.py). Credentials should come from environment
variables (see config.ALERT_EMAIL_SENDER / ALERT_APP_PASSWORD), never
hardcoded.

Usage:
    from alerts.email_alert import send_flood_alert
    ok, msg = send_flood_alert(
        flood_prob=0.87,
        upstream_vv=-9.2,
        rainfall=145.0,
        sender=config.ALERT_EMAIL_SENDER,
        password=config.ALERT_APP_PASSWORD,
