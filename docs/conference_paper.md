...
where T_climatology[month] is the empirical monthly mean derived from ERA5-Land 2009–2024 data (Table II). The anomaly correlation drops to r = −0.031 (p = 0.79, not significant), confirming the seasonal confound is eliminated while legitimate within-month thermal signals are preserved.

**Table II: Monthly Climatological Baseline (ERA5-Land, 2009–2024)**

| Month | Jan | Feb | Mar | Apr | May | Jun | Jul | Aug | Sep | Oct | Nov | Dec |
|-------|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|
| Mean °C | 17.0 | 19.4 | 22.0 | 24.8 | 25.4 | 27.1 | 27.3 | 27.5 | 28.3 | 27.0 | 25.4 | 20.1 |

**Accuracy impact:** On the 77 real-SAR events, using raw temperature, LOOCV accuracy was 88.3% and feature importance ranked temperature first (RF importance = 0.180). After deconfounding, accuracy improved to 89.6% and temperature importance dropped to rank 11 (importance = 0.021). This confirms the original high temperature importance was a spurious seasonal artifact, and removing it allowed the model to learn genuinely predictive features.
...
Secondary evaluation uses an **independent 45-event hold-out** (2017–2024) with Open-Meteo archive rainfall and physics-calibrated SAR proxies. These events are not used in LOOCV training.
...
| Precision | **84.8%** | 28/33 positive predictions correct |
| F1-Score | **86.2%** | Harmonic mean |
| AUC-ROC | **0.943** (77 real-SAR) / **0.910** (101 full) | Threshold-independent; computed via LOOCV |
...
|----------|--------|----------|---------|-------|
| LOOCV real-SAR (primary) | 77 | **89.6%** | **0.943** | ✅ Primary thesis metric |
| LOOCV full dataset | 101 | — | **0.910** | RF+XGB; all real+proxy events |
| LOOCV extended dataset | 131 | 87.8% | 94.1% | Mixed real+proxy SAR |
| 45-event hold-out | 45 | 80.0% | 87.9% | ⚠️ Proxy SAR, conservative |
| 5-fold CV (synthetic) | 101 | 99.7% | — | ⚠️ Do not cite: overfit |

### C. Impact of Temperature Deconfounding
...
- After correction: accuracy **89.6%** (+1.3 pp), temperature importance rank 11 (0.021)
...
**Table V: Comparison with Recent Bangladesh Flood ML Studies**
...
### F. Upstream Barak Proxy Validation
...
### G. Alert System Deployment
### H. Statistical Significance (McNemar Test)

To verify that the RF+XGB ensemble improvement over Logistic Regression is statistically significant and not due to chance, McNemar's test was applied to the matched LOOCV predictions (n = 101, same folds).

**Table VI: McNemar Test — RF+XGB vs Logistic Regression**

| | LR Correct | LR Wrong |
|---|---|---|
| **Ensemble Correct** | 83 | 3 |
| **Ensemble Wrong**   | 5 | 10 |

RF+XGB accuracy: **85.1%**  ·  Logistic Regression accuracy: **87.1%**

McNemar statistic: χ²(1) = 0.1250, **p = 0.7237** (continuity-corrected).

The improvement is **not statistically significant** at α = 0.05 (b = 5, c = 3; ensemble outperforms LR on 3 events where LR fails).
...
The final system achieves LOOCV accuracy 89.6%, AUC-ROC 0.943 (77 real-SAR events) / 0.910 (full 101-event LOOCV), and 80.0% on an independent hold-out, deployed as a real-time dashboard with Bengali community alert capability. The honest assessment is that these numbers represent performance on real satellite data — not synthetic, not inflated by seasonal proxies.
